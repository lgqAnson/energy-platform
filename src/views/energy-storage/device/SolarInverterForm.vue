    ...
    <div class="flex-1 overflow-y-auto p-6">
      <div class="grid grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="text-sm text-white/60">设备编码</label>
                <input v-if="mode !== 'view'" v-model="formData.deviceCode" type="text" placeholder="请输入"
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">所属场站</label>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">设备名称</label>
                <input v-if="mode !== 'view'" v-model="formData.deviceName" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.deviceName || '-' }}
                </div>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">型号规格</label>
                <input v-if="mode !== 'view'" v-model="formData.modelSpec" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.modelSpec || '-' }}
                </div>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">设备类型</label>
                <select v-if="mode !== 'view'" v-model="formData.deviceType"
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="text-sm text-white/60">额定功率 (W)</label>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">功率因数范围</label>
                <input v-if="mode !== 'view'" v-model="formData.powerFactorRange" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.powerFactorRange || '-' }}
                </div>
              </div>
              <div class="space-y-1">
                <label class="text-sm text-white/60">最大效率 (%)</label>
                <input v-if="mode !== 'view'" v-model="formData.maxEfficiency" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">总谐波畸变率 (%)</label>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">最大输入电流</label>
                <input v-if="mode !== 'view'" v-model="formData.maxInputCurrent" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.maxInputCurrent || '-' }}
                </div>
              </div>
              <div class="space-y-1">
                <label class="text-sm text-white/60">防护等级</label>
                <input v-if="mode !== 'view'" v-model="formData.protectionLevel" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">额定输出电压 (V)</label>
                <input v-if="mode !== 'view'" v-model="formData.ratedOutputVoltage" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.ratedOutputVoltage || '-' }}
                </div>
              </div>
              <div class="space-y-1">
                <label class="text-sm text-white/60">冷却方式</label>
                <select v-if="mode !== 'view'" v-model="formData.coolingMethod"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white focus:outline-none focus:border-primary">
                  <option value="">请选择</option>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">额定输出频率 (Hz)</label>
                <input v-if="mode !== 'view'" v-model="formData.ratedOutputFrequency" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.ratedOutputFrequency || '-' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- 并网信息 -->
          <div class="space-y-4">
            <h4 class="text-lg font-bold text-primary border-l-4 border-primary pl-3">并网信息</h4>
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="text-sm text-white/60">通信地址</label>
                <input v-if="mode !== 'view'" v-model="formData.communicationAddress" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">并网编号</label>
                <input v-if="mode !== 'view'" v-model="formData.gridNumber" type="text" placeholder="请输入"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.gridNumber || '-' }}
                </div>
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">通信协议</label>
                <input v-if="mode !== 'view'" v-model="formData.communicationProtocol" type="text" placeholder="如: IP67"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
              ...
              <div class="space-y-1">
                <label class="text-sm text-white/60">并网检测日期</label>
                <input v-if="mode !== 'view'" v-model="formData.gridTestDate" type="date"
                  class="w-full px-3 py-2 text-sm rounded bg-white/10 border border-white/20 text-white focus:outline-none focus:border-primary" />
                <div v-else class="px-3 py-2 text-sm rounded bg-white/5 text-white">
                  {{ formData.gridTestDate || '-' }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-6">
          <!-- 照片展示/上传 -->
          <div class="space-y-4">
            <h4 class="text-lg font-bold text-primary">逆变器</h4>
            <template v-if="mode !== 'view'">
            ...
            <div v-else-if="mode === 'view' && deviceImage"
              ...
              <img :src="deviceImage" alt="逆变器" class="w-full h-auto cursor-pointer"
...
const formData = reactive<SolarDeviceForm>({
  deviceCode: '',
  stationName: '',
  deviceName: '',
  modelSpec: '',
